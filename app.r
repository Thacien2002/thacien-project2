...
translations <- list(
  rw = list(  # Kinyarwanda
    ...
    full_docs_links_title = "Reba Inyandiko Zuzuye n'Amashusho"
    ,
...
# ==============================================================================
# Source utility scripts from the correct relative path.
# The working directory is set by run_dashboard.R.
source(here::here("R", "data_loader.R"), local = TRUE) # This script now loads both CSV and shapefile
source(here::here("R", "data_utils.R"), local = TRUE)
source(here::here("R", "gemini_utils.R"), local = TRUE)

tryCatch({
  # The shapefile is now loaded via data_loader.R, so we just check if it exists.
  if (!exists("rwanda_districts_sf") || is.null(rwanda_districts_sf)) {
    stop("Shapefile object 'rwanda_districts_sf' not found after sourcing data_loader.R")
  } # The success message is already in the loader script.
}, error = function(e) {
...
# --- DATA VALIDATION ---
# The data objects 'full_data' and 'regional_data' are now created by the sourced scripts.
regional_data <- create_regional_data()

# Validate loaded data
if (!exists("full_data") || !is.data.frame(full_data) || nrow(full_data) == 0) {
  stop("❌ FATAL ERROR: Data loading failed. The 'full_data' object is not a valid data frame or is empty. The application cannot start.")
} else {
...
server <- function(input, output, session) {
  ...
  # Observer for province chart drilldown
  observeEvent(event_data("plotly_click", source = "drilldown"), {
    d <- event_data("plotly_click", source = "drilldown")
    req(d, d$x)
    updateSelectInput(session, "selected_province", selected = d$x)
  })
  # Define reactive language early so UI renderers can use it safely
  ...
  output$rankings_narrative <- renderUI({
    ...
    trend_comparison_df <- full_data %>%
      ...
      filter(input$selected_province == "All Cities" | province == input$selected_province) %>%
      group_by(year) %>%
  ...
  rankings_data <- reactive({
    ...
    data_for_ranking %>%
      group_by(district, province) %>% # Already filtered, so this summarizes correctly
      summarise(
      ...
      left_join(trend_data, by = "district") %>% # Join trend data
      arrange(avg_indicator, poverty_rate) %>% # Arrange by primary indicator, then by poverty rate
      mutate(
  ...
  output$rankings_map <- renderLeaflet({
    ...
    leaflet(data = map_data_sf) %>%
      ...
      addLegend(
        ...
        title = paste(tools::toTitleCase(gsub("_", " ", indicator)), "(%)"),
        opacity = 1
      )
  })
  ...
  output$resource_allocation <- renderPlotly({
    ...
    p <- plot_ly(allocation_data, 
                 ...
                 colors = c("High Priority" = "#ef4444", "Medium Priority" = "#f97316", "Low Priority" = "#eab308"), 
                 type = 'bar') %>%
      layout(
  ...
  output$districts_improved <- renderInfoBox({
    ...
    comparison_df <- full_data %>%
      ...
      filter(input$selected_province == "All Cities" | province == input$selected_province) %>%
      filter(input$selected_age_group == "All Age Groups" | age_group == input$selected_age_group) %>%
      filter(input$selected_nutrient == "All Nutrients" | nutrient == input$selected_nutrient)
